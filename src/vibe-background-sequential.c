...
// -----------------------------------------------------------------------------
// Counting proportion of mvt pixels in segmentation map
// -----------------------------------------------------------------------------
int32_t countMvtPixels(vibeModel_Sequential_t *model, uint8_t *segmentation_map)
	{
	 /* Basic checks. */
	assert((model != NULL) && (segmentation_map != NULL));
	...
	uint32_t nbMvtPixels = 0;

	/* Counting mvt pixels. */
	for (int index = width * height - 1; index >= 0; --index) {
	    if (segmentation_map[index] >= matchingNumber-1)
	    	nbMvtPixels++;
	}
	return(nbMvtPixels);
}
